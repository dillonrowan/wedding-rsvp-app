...
export default async function RsvpGroupSearchResultBody(props) {
    ...
    return (
        <>
            {
                props.rsvpGroups?.length > 1 ? <p>We've found more than one match in the guest list. Please select your name from the list below.</p>
                    ...
                    <form onSubmit={(e) => { handleSubmit(e) }}>
                        {props.rsvpGroups?.map((rsvpGroup) => (                        
                            <div key={rsvpGroup.id}>
                                <input type="radio" name="rsvp-group" value={JSON.stringify(rsvpGroup)} onChange={(e) => { temp = e.target.value }}></input>
